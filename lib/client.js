...
window.__ModuleLoader__.load({
    ...
    `;
        ...
        function copyModels(value, fallbackMode, fallbackSummary) {
        ...
        function ModelRow(props) {
            ...
            return e('div', { className: 'rm-model' }, e('label', { className: 'rm-model-main' }, e('input', { type: 'checkbox', checked, disabled, onChange: onToggle }), e('span', { className: 'rm-model-copy' }, e('span', { className: 'rm-model-name' }, modelName), e('span', { className: 'rm-route' }, `${providerName} · ${item.provider}/${item.model}`))), e('label', { className: 'rm-select-wrap' }, e('span', { className: 'rm-select-label' }, t('mode')), e('select', { className: 'rm-select', value: mode, disabled: disabled || !checked, 'aria-label': `${t('mode')}: ${item.provider}/${item.model}`, onChange: (event) => onChange({ mode: event.target.value }) }, e('option', { value: 'standard' }, t('standard')), e('option', { value: 'pro' }, t('pro')))), e('label', { className: 'rm-select-wrap' }, e('span', { className: 'rm-select-label' }, t('summary')), e('select', { className: 'rm-select', value: summary, disabled: disabled || !checked, 'aria-label': `${t('summary')}: ${item.provider}/${item.model}`, onChange: (event) => onChange({ summary: event.target.value }) }, e('option', { value: 'auto' }, t('auto')), e('option', { value: 'concise' }, t('concise')), e('option', { value: 'detailed' }, t('detailed')))), !available ? e('span', { className: 'rm-unavailable' }, t('unavailableRoute')) : null);
        }
        function ReasoningModeCard(props) {
            ...
            const draftByKey = new Map(draftModels.map((item) => [keyOf(item), item]));
            ...
            const updateModel = (item, patch) => {
                const key = keyOf(item);
                setDraftModels((current) => current.map((entry) => keyOf(entry) === key ? { ...entry, ...patch } : entry));
                setDirty(true);
            };
            const save = async () => {
            ...
            const renderRow = (providerName, item, available, modelName) => {
                const key = keyOf(item);
                ...
                return e(ModelRow, {
                    ...
                    onChange: (patch) => updateModel(item, patch),
                    t,
                });
            };
            return e('li', { className: `rm-card ${open ? 'rm-card-open' : ''}` }, e('button', {
                type: 'button', className: 'rm-head', 'aria-expanded': open,
                'aria-label': `${t(open ? 'collapse' : 'expand')}: ${t('title')}`,
                onClick: () => setOpen(!open),
            }, e('span', { className: 'rm-heading' }, e('span', { className: 'rm-name' }, t('title')), e('span', { className: 'rm-description' }, t('description'))), dirty ? e(Tag, { tone: 'neutral', className: 'rm-pending' }, t('unsaved')) : null, e(IconChevronDownOutline14, { className: `rm-chevron ${open ? 'rm-chevron-open' : ''}` })), open ? e('div', { className: 'rm-body' }, !snapshot.writable ? e('p', { className: 'rm-readonly', role: 'status' }, t('readOnly')) : null, e('div', { className: 'rm-field' }, e('div', { className: 'rm-field-head' }, e('span', { className: 'rm-field-label' }, t('defaults'))), e('p', { className: 'rm-hint' }, t('routeHint')), e('div', { className: 'rm-defaults' }, e('label', { className: 'rm-select-wrap' }, e('span', { className: 'rm-select-label' }, t('mode')), e('select', { className: 'rm-select', value: draftMode, disabled: !snapshot.writable || saving, onChange: (event) => { setDraftMode(event.target.value); setDirty(true); } }, e('option', { value: 'standard' }, t('standard')), e('option', { value: 'pro' }, t('pro')))), e('label', { className: 'rm-select-wrap' }, e('span', { className: 'rm-select-label' }, t('summary')), e('select', { className: 'rm-select', value: draftSummary, disabled: !snapshot.writable || saving, onChange: (event) => { setDraftSummary(event.target.value); setDirty(true); } }, e('option', { value: 'auto' }, t('auto')), e('option', { value: 'concise' }, t('concise')), e('option', { value: 'detailed' }, t('detailed')))))), e('div', { className: 'rm-field' }, e('div', { className: 'rm-field-head' }, e('span', { className: 'rm-field-label' }, t('models')), e('span', { className: 'rm-count' }, t('selected', draftModels.length))), e('p', { className: 'rm-hint' }, t('modelsHint')), catalogError !== null ? e('div', { className: 'rm-catalog-error', role: 'alert' }, e('span', null, `${t('catalogFailed')} (${catalogError})`), e('button', { type: 'button', disabled: saving, onClick: () => { void loadCatalog(); } }, t('retry'))) : null, catalog === null && catalogError === null ? e('p', { className: 'rm-notice', role: 'status' }, t('loading')) : null, (catalog && catalog.length > 0) || unavailable.length > 0 ? e('fieldset', { className: 'rm-models' }, e('legend', null, t('models')), catalog?.map((group) => e('div', { className: 'rm-model-group', key: group.id }, e('div', { className: 'rm-provider' }, group.name ?? group.id), (group.models ?? []).map((model) => renderRow(group.name ?? group.id, { provider: group.id, model: model.id }, true, displayName(model))))), unavailable.length > 0 ? e('div', { className: 'rm-model-group' }, e('div', { className: 'rm-provider' }, t('unavailableGroup')), unavailable.map((item) => renderRow(item.provider, item, false, item.model))) : null) : null, catalog && catalog.length === 0 && unavailable.length === 0 ? e('p', { className: 'rm-notice' }, t('noModels')) : null), e('div', { className: 'rm-footer' }, failed ? e('p', { className: 'rm-failed', role: 'status' }, t('saveFailed')) : null, e('button', { type: 'button', className: 'rm-discard', disabled: !dirty || saving, onClick: discard }, t('discard')), e('button', { type: 'button', className: 'rm-save', disabled: !dirty || saving || !snapshot.writable, onClick: () => { void save(); } }, saving ? t('saving') : t('save')))) : null);
        }
        ...
        function ModeControl(props) {
            ...
            const option = (value, label, current, patch) => e('button', {
            ...
            }, e('span', { className: 'rm-menu-option-copy' }, e('span', { className: 'rm-menu-option-label' }, label)), e('span', { className: 'rm-menu-check', 'aria-hidden': true }, current === value ? e(IconCheckOutline16, null) : null));
            const cell = (label, value, nextPane) => e('button', {
                type: 'button', className: 'rm-menu-cell', role: 'menuitem', 'aria-haspopup': 'menu',
                onClick: () => setPane(nextPane),
            }, e('span', { className: 'rm-menu-cell-label' }, label), e('span', { className: 'rm-menu-cell-value' }, value), e(IconChevronRightOutline14, { className: 'rm-menu-cell-chevron' }));
            const menu = e('div', {
            ...
            }, pane === 'root' ? e(React.Fragment, null, cell(t('modeLabel'), modeLabel, 'mode'), cell(t('summaryLabel'), summaryLabel, 'summary')) : pane === 'mode' ? e(React.Fragment, null, option('standard', t('standard'), config.mode, { mode: 'standard' }), option('pro', t('pro'), config.mode, { mode: 'pro' })) : e(React.Fragment, null, option('auto', t('auto'), config.summary, { summary: 'auto' }), option('concise', t('concise'), config.summary, { summary: 'concise' }), option('detailed', t('detailed'), config.summary, { summary: 'detailed' })));
            const menuNode = ReactDOM && typeof ReactDOM.createPortal === 'function' && typeof document !== 'undefined'
                ? ReactDOM.createPortal(menu, document.body)
                : menu;
            return e('div', { className: 'rm-control-root', ref: rootRef }, e('button', {
                ref: triggerRef,
            ...
            }, e('span', { className: 'rm-control-value' }, modeLabel), e('span', { className: 'rm-control-separator', 'aria-hidden': true }, '·'), e('span', { className: 'rm-control-value' }, summaryLabel), e(IconChevronDownOutline14, { className: `rm-control-chevron ${open ? 'rm-control-chevron-open' : ''}` })), open ? menuNode : null);
        }
        function apply(ctx) {
            ...
            }, (props) => e(ReasoningModeCard, { ...props, scope, sessionFace, t })));
            slots.inject('conversation.input.right', () => slots.register({
                name: 'conversation.input.right',
                id: NS,
                order: 85,
                locale: NS,
            }, (props) => e(ModeControl, { ...props, scope, t })));
        }
